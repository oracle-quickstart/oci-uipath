...
function Install-DotNetFramework {
  ...
  $exitCode = 0
  $argumentList = "/q /L*vx+ donnet.log /norestart"
  ...
  if ($exitCode -ne 0) {
    Log-Error -LogPath $sLogFile -ErrorDesc "Failed to install .Net Framework  4.7.2(Exit code: $exitCode). Higher or equal version may already be installed" -ExitGracefully $False
  }
  else {
...
function Install-DotNetHostingBundle {
  ...
  $exitCode = 0
  $argumentList = "OPT_NO_SHARED_CONFIG_CHECK=1 /q /L*vx+ hostingbundle.log /norestart"
